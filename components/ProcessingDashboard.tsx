    ...
    <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-slate-950/95 backdrop-blur-xl">
      <div className="w-full max-w-4xl bg-slate-900 border border-white/10 rounded-[3rem] overflow-hidden shadow-[0_0_80px_rgba(0,0,0,0.6)] flex flex-col md:flex-row h-[80vh]">
        ...
        <div className="md:w-1/3 bg-slate-950 p-10 border-r border-white/5 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                ...
                <p className="text-slate-500 text-[9px] font-black uppercase tracking-widest">Dashboard v1.0</p>
              </div>
            </div>
            
            <div className="space-y-6">
              <div className="p-5 bg-white/5 border border-white/5 rounded-2xl">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Target Region</p>
          ...
          <button 
            onClick={onClose}
            className="w-full py-4 border-2 border-white/5 text-slate-500 hover:text-white hover:border-white/10 rounded-2xl font-black uppercase tracking-widest transition-all"
          >
            Fechar Dashboard
          </button>
        </div>
        ...
        <div className="flex-grow p-10 flex flex-col">
          <div className="flex justify-between items-center mb-10">
            ...
            <div className={`px-4 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${apiStatus === 'ACTIVE' ? 'bg-orange-500/20 text-orange-500 animate-pulse' : 'bg-slate-800 text-slate-500'}`}>
          ...
          <div className="flex-grow flex flex-col gap-10">
            ...
            <div className="bg-slate-950 p-8 rounded-[2rem] border border-white/5">
              <div className="flex justify-between mb-4">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Status da Fila de Indexação</span>
            ...
            <div className="flex-grow bg-black p-6 rounded-[2rem] border border-white/5 font-mono overflow-y-auto custom-scrollbar">
              {logs.length === 0 ? (
                <p className="text-slate-700 text-[11px] italic">Aguardando comando do administrador...</p>
              ) : (
                logs.map((log, i) => (
                  <p key={i} className="text-[11px] mb-2">
                    <span className="text-emerald-500">{">"}</span> <span className="text-slate-400">{log}</span>
                  </p>
                ))
              )}
            </div>
