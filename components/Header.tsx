      ...
      <div className="max-w-7xl mx-auto px-4 flex justify-between items-center">
        ...
        <nav className="hidden lg:flex gap-8 items-center">
          ...
          <div className="flex gap-4">
            <button onClick={onPatientOpen} className="text-[10px] font-black text-blue-600 hover:bg-blue-50 transition-colors uppercase tracking-widest px-4 py-2 border-2 border-blue-600 rounded-xl">Portal Paciente</button>
            <button onClick={onDoctorOpen} className="text-[10px] font-black text-white bg-slate-900 hover:bg-black transition-colors uppercase tracking-widest px-4 py-2 rounded-xl border border-white/10 shadow-lg">Credenciamento Médico</button>
          </div>
        </nav>
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 bg-white border-b border-slate-200 p-6 space-y-4 animate-fade-in shadow-2xl">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => { onPatientOpen(); setIsMobileMenuOpen(false); }} className="py-4 border-2 border-blue-600 text-blue-600 rounded-2xl text-[10px] font-black uppercase tracking-widest">Paciente</button>
            <button onClick={() => { onDoctorOpen(); setIsMobileMenuOpen(false); }} className="py-4 bg-slate-100 text-slate-700 rounded-2xl text-[10px] font-black uppercase tracking-widest">Médico</button>
          </div>
