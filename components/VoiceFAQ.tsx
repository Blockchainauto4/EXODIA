    ...
    <section id="faq" className="py-24 bg-slate-50">
      <div className="max-w-4xl mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-700 font-bold italic text-base tracking-tight">"Ok Google, onde tem {spec.toLowerCase()} aqui perto em {city}?"</p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8">
          {faqs.map((faq, idx) => (
            <div key={idx} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 hover:border-blue-400 transition-all duration-300 group">
              ...
              <p className="text-slate-700 text-sm leading-relaxed ml-16 font-medium border-l-2 border-slate-200 pl-6 group-hover:border-blue-300">{faq.a}</p>
            </div>
          ))}
        </div>

        <div className="mt-16 text-center">
          <p className="text-xs font-black text-slate-700 uppercase tracking-[0.3em] mb-6">Explore em {city}:</p>
